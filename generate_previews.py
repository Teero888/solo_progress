...
PREVIEWS_DIR = Path('public/previews')
...
def generate_previews():
    ...
    abs_previews = PREVIEWS_DIR.resolve()

    for i, map_path in enumerate(map_files, 1):
        map_name = map_path.stem
        target_file = abs_previews / f"{map_name}.png"
        if target_file.exists():
            print(f"[{i}/{total_maps}] Skipping {map_name} (preview already exists)")
            continue

        print(f"[{i}/{total_maps}] Generating preview for {map_name}...")
        cmd = [
            str(abs_binary),
        ...
        try:
            result = subprocess.run(cmd, cwd=abs_previews, check=True, capture_output=True, text=True)
            output_file = abs_previews / f"{map_name}_{RESOLUTION}.png"
            ...
            print(f"  Error generating preview for {map_name}:")
